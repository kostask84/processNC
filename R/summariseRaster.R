...
summariseRaster <- function(files, startdate=NA, enddate=NA, ext=NA, var,
  ...
  if(overwrite==FALSE & file.exists(filename1)){
    ...
    if(var %in% c("hurs", "huss", "tas", "sfcWind")){
      avg <- raster::zApply(data, by=yearmon, fun=mean, name='months', na.rm=TRUE)
    } else if(var == "pr"){
      avg <- raster::zApply(data, by=yearmon, fun=sum, name='months', na.rm=TRUE)
    } else if(var == "tasmax"){
      avg <- raster::zApply(data, by=yearmon, fun=max, name='months', na.rm=TRUE)
    } else if(var == "tasmin"){
      avg <- raster::zApply(data, by=yearmon, fun=min, name='months', na.rm=TRUE)
    }
    ...
    if(filename2 != ""){
      cv <- raster::zApply(data, by=yearmon, fun=cv, name='months', na.rm=TRUE)
